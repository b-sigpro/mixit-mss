...
from mixit_mss.common.datasets.hdf5_wav_dataset import HDF5WavDataset
from mixit_mss.common.datasets.hdf5_wav_dataset_dm import HDF5WavDMDataset
            ...
            if self.use_scatter_dataset and torch.cuda.device_count() > 1:
                self.train_dataset = scatter_dataset(
                    train_dataset_class(
                        ...
                        **self.default_train_dataset_kwargs,
                    )
                )
                self.val_dataset = scatter_dataset(
                    HDF5WavDataset(
                        self.val_dataset_path,
                        **self.default_val_dataset_kwargs,
                    )
                )
            else:
