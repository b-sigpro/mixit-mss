...
class EfficentMixITWrapper(nn.Module):
    ...
    def forward(self, est_targets, targets, **kwargs):
        ...
        perm_mat = EfficentMixITWrapper.find_best_perm_with_least_square(est_targets, targets)
    ...
    def find_best_perm_with_least_square(est_targets, targets):
        ...
        # get permutation matrix via least-square
        if est_targets.ndim == 4:
            # when input is multi-channel, we only use 0-th channel
            # to get the permutation matrix
            A = torch.linalg.lstsq(est_targets[..., 0, :, :], targets[..., 0, :, :])[0]
            A = A.unsqueeze(1).repeat(1, est_targets.shape[1], 1, 1)
        else:
        ...
        perm_mat = (Amax == A).to(est_targets.dtype)
