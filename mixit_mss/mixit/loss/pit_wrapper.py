...
class PITLossWrapper(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        mean_loss = torch.mean(min_loss) if return_mean else min_loss

        if not return_est:
            return mean_loss
        reordered = self.reorder_source(est_targets, batch_indices)
        if return_perm:
            return mean_loss, reordered, batch_indices
        else:
            return mean_loss, reordered
    ...
    def find_best_perm_hungarian(pair_wise_losses: torch.Tensor):
        ...
        batch_indices = torch.tensor([linear_sum_assignment(pwl)[1] for pwl in pwl_copy]).to(pwl.device)
