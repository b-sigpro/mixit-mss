...
from mixit_mss.mixit.loss.pit_wrapper import PITLossWrapper
...
class ThresSNRLoss(nn.Module):
    ...
    def pairwise_thresSNR(self, est, ref, eps=1e-8):
        ...
        noise = ((ref - est) ** 2).sum(dim=-1)
        tgt = (ref**2).sum(dim=-1)
        threshold = ((ref.sum(dim=-2)) ** 2).sum(dim=-1, keepdim=True) if self.threshold_with == "mixture" else tgt
    ...
    def forward(
        ...
        solve_perm = self.solve_perm if solve_perm is None else solve_perm

        # if input shape is (n_batch, n_time)
        if ref.ndim == 2 or not solve_perm:
...
def snr(
    ...
    if solve_perm:
        perms = list(itertools.permutations(range(n_est), n_ref))
    else:
        assert n_est == n_ref, (est.shape, ref.shape)
